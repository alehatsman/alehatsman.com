export const meta = { 
  ...
  description: 'Infrastructure as code is de facto standard in the world of devops. But why not to use the same tools for your personal laptop. Got a new mac, run Ansible playbooks and have a ready to use MacBook.', 
...
Got a new mac? Don't want to spend the whole weekend sitting and installing your favourite apps? Automate that process! That what I did in my [osx-automation](https://github.com/atsman/osx-automation) project.
...
In simple words, configuration is stored in text files, text files are stored in git repository. And thanks to that, you always have access to those configs and to the change history.

Why not to use the same approach to your personal laptop. Put all configuration in text files, and store those files in GitHub. That's what people do with **dotfiles**.
