...
Most of the time I spend in command line, writing code in **Nvim**, executing commands in **Zsh** shell, switching panes and sessions in **Tmux**  and all that is rendered in **Alacritty** terminal emulator running on my Mac.
...
Most of the time I spend in command line, reading and writing code in `Nvim`, jumpling between terminal sessions in `Tmux` and running commands in `Zsh` and all that is rendered in `Alacritty` terminal emulator.
...
At the start of my career, my terminal usage mostly was to run a script to compile code, run tests or interact with git. For quite a long period of time I was fun of terminal that is integrated in my editor.

Some forces attracted me into world of text based apps and instead of having terminal in the Editor, I'm having Editor in the terminal.

When you havily use command line, espessially when you search, move around, or write code in Nvim - you expect you terminal to render text fast. At some point performance of my terminal frustrated me and I found a solution - [Alacritty][alacritty link].
...
Alacritty is configured using using global text file located in `~/.config/alacritty/alacritty.yml`

Config line below fixes colors rendering:
...
Each time returning back to work after long travel and opening laptop with my carifully configured setup, I feel like a kid who just got his new toy.
